...
# Função para enumerar e plotar veículos detectados em um quadro
def enumerar_e_plotar_veiculos(img, histogramas_com_coordenadas, quadro_numero, pasta_run):
    for numero_veiculo, (_, (x1, y1, x2, y2)) in enumerate(histogramas_com_coordenadas, start=1):
        # Adiciona o número do veículo na imagem e desenha um retângulo ao redor da detecção
        cv2.putText(img, str(numero_veiculo), (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
        cv2.rectangle(img, (x1, y1), (x2, y2), (0, 255, 0), 2)
    
    # Salva o quadro com os veículos enumerados
    cv2.imwrite(os.path.join(pasta_run, f"quadro_{quadro_numero}_enumerado.png"), img)
    print(f"Quadro {quadro_numero+1} com veículos enumerados salvo em {pasta_run}")
...
def main():
    ...
    for quadro in range(len(quadros) - 1):
        ...
        for numero_veiculo, (histograma_atual, (x1, y1, x2, y2)) in enumerate(histogramas_atual_quadro, start=1):
            ...
            # Chama a função para enumerar e plotar veículos no quadro atual
            enumerar_e_plotar_veiculos(img_atual, histogramas_atual_quadro, quadro, pasta_run)
