...
#Iterar até len(even_frames) - 1 para incluir o último par de frames
for i in range(len(even_frames) - 1):
    ...
    detected_vehicles_frame1 = model(source=frame1_path, classes=(2, 5, 7))
    detected_vehicles_frame2 = model(source=frame2_path, classes=(2, 5, 7))
    ...
    for j, box in enumerate(detected_vehicles_frame1[0].boxes.xyxy):
        x1, y1, x2, y2 = map(int, box)
    ...
    enumeration_map_frame2 = [-1] * len(detected_vehicles_frame2[0].boxes.xyxy)
    
    for j, box in enumerate(detected_vehicles_frame2[0].boxes.xyxy):
        x1, y1, x2, y2 = map(int, box)
    ...
    save_vehicles_with_enumeration(image1, detected_vehicles_frame1[0].boxes.xyxy, list(range(len(detected_vehicles_frame1[0].boxes.xyxy))), frame_name=f'detected_frame_{even_frames[i]}', output_folder=pair_folder)
    save_vehicles_with_enumeration(image2, detected_vehicles_frame2[0].boxes.xyxy, enumeration_map_frame2, frame_name=f'detected_frame_{even_frames[i + 1]}', output_folder=pair_folder)

    # Salva também cada frame na pasta 'all_frames_with_detections' usando o mapeamento global
    save_vehicles_with_enumeration(image1, detected_vehicles_frame1[0].boxes.xyxy, list(range(len(detected_vehicles_frame1[0].boxes.xyxy))), frame_name=f'detected_frame_{even_frames[i]}', output_folder=all_frames_folder)
    save_vehicles_with_enumeration(image2, detected_vehicles_frame2[0].boxes.xyxy, enumeration_map_frame2, frame_name=f'detected_frame_{even_frames[i + 1]}', output_folder=all_frames_folder)
