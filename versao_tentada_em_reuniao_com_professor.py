...
def processar_quadros(model, quadros, pasta_quadros, pasta_run):
    for i in range(len(quadros) - 1):
        ...
        numero_quadro = i + 1
        for numero_veiculo, (histograma_atual, coordenada_atual, id_atual)  in enumerate(histogramas_atual, start=1):
            menor_distancia = float('inf')
            histograma_semelhante = None 
            coordenada_semelhante = None

            for histograma_proximo, coordenada_proximo, id_proximo in histogramas_proximo:
            ...
            enumera_veiculo_atual_e_seu_semelhante_com_o_mesmo_numero(img_atual, img_proximo, coordenada_atual, coordenada_semelhante, id_proximo, pasta_run)
